...
struct Point {
  ...
  void* node_ptr;  // 이 점이 속한 node의 pointer. Point q에 대한 matching
                   // p_match 에 대해서만 할당해준다.
  Point() {
    u = 0;
    v = 0;
    idx = 0;
...
struct Node {
  ...
  Node(Corner topleft_, Corner botright_, Point pt_, Node* parent_,
       int depth_) {
    topleft = topleft_;
    botright = botright_;
    center =
        Corner((topleft.u + botright.u) * 0.5, (topleft.v + botright.v) * 0.5);
...
class QuadTree {
 ...
 public:  // methods
  ...
  QuadTree(std::vector<Point> pts_input, double n_rows_, double n_cols_,
           int max_depth_, double eps_);
  ...
  void cachedNNSearchMultiple(std::vector<Point> qs,
...
QuadTree::QuadTree(double n_rows_, double n_cols_, int max_depth_,
                   double eps_) {
...
QuadTree::QuadTree(std::vector<Point> pts_input, double n_rows_, double n_cols_,
                   int max_depth_, double eps_) {
  QuadTree::createRoot(n_rows_, n_cols_);
  max_depth = max_depth_;
  eps = eps_;
...
void QuadTree::createRoot(double n_rows_, double n_cols_) {
  root = new Node(Corner(0, 0), Corner(n_cols_, n_rows_), Point(-1, -1, -1),
                  NULL, 0);  // this is root node.
  topleft_root = Corner(0, 0);
  botright_root = Corner(n_cols_, n_rows_);
  root->isleaf = false;
};

bool QuadTree::inBoundary(
    Point p) {  // 입력해주는 점이 global boundary안에 있는지 확인한다.
  return (p.u >= topleft_root.u && p.u <= botright_root.u &&
          p.v >= topleft_root.v && p.v <= botright_root.v);
};

bool QuadTree::BWBTest(
    Node* node, Point q,
    double dist) {  // leaf에서 이거 통과하면 지금의 매칭이 리얼 매칭이다.
...
void QuadTree::insert(Node* node, Point p,
                      int depth) {  // 맨 처음 넣어주는 Node*는 root 이다.
  if (depth ==
      max_depth) {  // 최대 깊이이면, 더이상 진행 불가하고, pts에 넣어준다.
    node->isleaf = true;
    p.node_ptr = (void*)node;  // 이제부터 이 점의 소속은 node이다.
    node->pts.push_back(p);
    return;
  } else {  // 최대 깊이가 아니다. 아래 node가 더 있을 수 있다. (해당 라인은
            // 최대 depth - 1 까지만 도달가능.)
    if (node->isleaf) {  // 현재 노드가 leaf이면 leaf의 점도 같이 가지고
                         // 내려간다.
      node->isleaf =
          false;  // 이제부터는 자식 노드가 생기므로 더이상 leaf가 아니다.
      // 1) node->pt 를 여행시켜준다. 자식이 없으므로 자식 생성해줘야하고, 바로
      // 아래 child로 갈 뿐이다.
      if (node->center.u >=
          node->pt.u) {  // west(left), 당연히 아무것도 없으니까 새노드 생성.
        if (node->center.v >= node->pt.v) {  // north west
          node->child_tl =
              new Node(node->topleft, node->center, node->pt, node, depth + 1);
          node->child_tl->pt.node_ptr = (void*)node->child_tl;
        } else {  // south west
          node->child_bl = new Node(Corner(node->topleft.u, node->center.v),
                                    Corner(node->center.u, node->botright.v),
                                    node->pt, node, depth + 1);
          node->child_bl->pt.node_ptr = (void*)node->child_bl;
        }
      } else {  // east (right), 당연히 아무것도 없으니까 새노드 생성.
        if (node->center.v >= node->pt.v) {  // north east
          node->child_tr = new Node(Corner(node->center.u, node->topleft.v),
                                    Corner(node->botright.u, node->center.v),
                                    node->pt, node, depth + 1);
          node->child_tr->pt.node_ptr = (void*)node->child_tr;
        } else {  // south east
          node->child_br =
              new Node(node->center, node->botright, node->pt, node, depth + 1);
      ...
      if (node->center.u >= p.u) {    // west(left)
        if (node->center.v >= p.v) {  // north west
          if (node->child_tl ==
              NULL) {  // 아래에 아무것도 안들어있다면, 넣어주고 leaf가 된다.
            node->child_tl =
                new Node(node->topleft, node->center, p, node, depth + 1);
            node->child_tl->pt.node_ptr = (void*)node->child_tl;
        ...
        if (node->center.v >= p.v) {  // north east
          ...
          if (node->child_br ==
              NULL) {  // 아래에 아무것도 안들어있다면, 넣어주고 leaf가 된다.
            node->child_br =
                new Node(node->center, node->botright, p, node, depth + 1);
            node->child_br->pt.node_ptr = (void*)node->child_br;
          } else  // 자식노드가 있다면, insert 함수 재호출.
...
void QuadTree::cachedNNSearchMultiple(std::vector<Point> qs,
                                      std::vector<void*> cached_nodes,
                                      std::vector<int>& idxs) {
  int npoints = qs.size();
  Point p_match;
...
Point QuadTree::cachedNNSearch(Node* node, Point q) {
  ...
  if (node_now->depth != 0) {  // root 인 경우는 패스.
    while (!QuadTree::BWBTest(node_now, q,
                              min_dist))  // BWBTest가 될 때까지 위로 올라간다.
      node_now = node_now->parent;
  }

  // (3) q를 contain 하는 node로 다시 찾아 내려간다. 가는 동안 BOB 테스트 같은건
  // 필요없다.
...
/* // 사실 필요한건지 모르겠다.
bool QuadTree::BOBTest(Node* node, Point q, double ball_radius){ // BOB? BWB가
만족되지 않으면 계속 올라가는데,
    ...
    if(q.u <= node->topleft.u){ // left
        if(q.v <= node->topleft.v){ // left top( north west 방향에 점이 위치)
            dist_temp = (node->topleft.u - q.u)*(node->topleft.u - q.u) +
(node->topleft.v - q.v)*(node->topleft.v - q.v); if(ball_radius >= dist_temp) //
ball_radius 가 더 크면 overlap. return true; else return false;
        }
        else{ // left bot( south west 방향에 점이 위치)
            dist_temp = (node->topleft.u - q.u)*(node->topleft.u - q.u) +
(node->botright.v - q.v)*(node->botright.v - q.v); if(ball_radius >= dist_temp)
// ball_radius 가 더 크면 overlap. return true; else return false;
        }
    }
    else{ // right
        if(q.v <= node->topleft.v){ // right top( north east 방향에 점이 위치)
            dist_temp = (node->botright.u - q.u)*(node->botright.u - q.u) +
(node->topleft.v - q.v)*(node->topleft.v - q.v); if(ball_radius >= dist_temp) //
ball_radius 가 더 크면 overlap. return true; else return false;
        }
...
/* bool QuadTree::nearestNeighborSearch(Node* node, Point q, Point& p_match,
double& min_dist){ // 하나의 query point q에 대해서 검색한다.
    ...
    if(node->isleaf){ // leaf node이면 비교해 볼 point들이 존재한다.
        ...
        if(BWBTest(node, q, *min_dist)){
            return true; // 완전히 모든 연산이 끝났다. 계속 타고 나가면서 재귀를
collapse.
        }
        else{
    ...
    else{ // leaf가 아닌경우, 아래로 traverse. 단 BOB test가 만족되는 경우에만
간다. 아닌 경우에는 갈 이유가 없다. if(node->center.u >= q.u){ // left
