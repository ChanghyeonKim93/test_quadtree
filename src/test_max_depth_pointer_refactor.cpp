...
int main() {
  ...
  for (int i = 0; i < num_query_point; ++i) {
    ...
    for (int j = 0; j < num_reference_point; ++j) {
      ...
      const auto distance = DIST_EUCLIDEAN(
          query_point.u, query_point.v, reference_point.u, reference_point.v);
  ...
  try {
    ...
    for (int d = 0; d <= max_tree_depth; ++d) {
      ...
      quadtree = std::make_shared<Quadtree>(
          x_range[0], x_range[1], y_range[0], y_range[1], d,
          max_num_element_in_leaf, approx_rate);
      ...
      for (uint32_t i = 0; i < num_reference_point; ++i) {
        ID id_data = i;
        quadtree->InsertReferenceData(reference_point_list[i].u,
                                      reference_point_list[i].v, id_data);
      }
      std::cout << "# nodes - activated: " << quadtree->GetNumActivatedNodes()
                << std::endl;
      ...
      for (uint32_t i = 0; i < num_query_point; ++i) {
        ...
        quadtree->SearchNearestNeighbor_debug(
            ...
            matched_reference_id_list[i], matched_node_ptr_list[i],
            access_temp);
      ...
      for (int i = 0; i < num_query_point; ++i) {
        ...
        if (id_mat != id_mat_true) {
          ...
          std::cout << "mismatched: " << id_mat << "," << id_mat_true << " / "
                    << reference_point_list[id_mat_true].u << ","
                    << reference_point_list[id_mat_true].v << " / "
                    << reference_point_list[id_mat].u << ","
                    << reference_point_list[id_mat].v
                    << " / mindist2 (true,est):" << true_min_distance_list[i]
                    << ","
                    << DIST_EUCLIDEAN(reference_point_list[id_mat].u,
                                      reference_point_list[id_mat].v,
                                      query_point_list[i].u,
                                      query_point_list[i].v)
                    << std::endl;
        }
      }
      ...
      for (int i = 0; i < num_query_point; ++i) {
        ...
        quadtree->SearchNearestNeighborWithNodeCache_debug(
            query_point_list[i].u, query_point_list[i].v,
            matched_node_ptr_list[i], matched_reference_id_list[i],
            matched_node_ptr_list[i], access_temp);
    ...
    for (int depth = 0; depth <= max_tree_depth; ++depth) {
      std::cout << " d[" << depth << "] - "
                ...
                << "NN search (normal/cached): " << time_normal[depth] << ", "
                << time_cached[depth] << "\n";
    }
    std::cout << "==== ACCESS ANALYSIS ====\n";
    for (int depth = 0; depth < max_tree_depth; ++depth) {
      std::cout << " d[" << depth << "] - (normal/cached)"
                << "total: " << access_normal[depth] << ", "
                << access_cached[depth] << " / "
                << "min: " << min_access_normal[depth] << ","
                << min_access_cached[depth] << " / "
                << "max: " << max_access_normal[depth] << ","
                << max_access_cached[depth] << " / "
                << "avg: " << avg_access_normal[depth] << ","
                << avg_access_cached[depth] << " / "
                << "std: " << std_access_normal[depth] << ","
                << std_access_cached[depth] << std::endl;
    }
