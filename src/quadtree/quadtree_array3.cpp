...
namespace ArrayBased3
{
    ...
    inline bool Quadtree::BOBTest(float x, float y, const Rectangle<_numeric_uint> &rect, float R)
    {
        ...
        if (dx > (rect_halfsize_x + R) || dy > (rect_halfsize_y + R))
            return false;

        if (dx <= rect_halfsize_x || dy <= rect_halfsize_y)
            return true;
    ...
    void Quadtree::nearestNeighborSearchPrivate()
    {
        ...
        while (!simple_stack_.empty())
        {
            ...
            { // this is not a leaf node.
                ...
                if (nodes_[id_child].isActivated() && BOBTest(x_nom, y_nom, nodes_[id_child].rect, query_data_.min_dist_))
                {
