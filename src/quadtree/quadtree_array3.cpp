#include "quadtree/quadtree_array3.h"
...
Quadtree::Quadtree(float x_min, float x_max, float y_min, float y_max,
    ...
    : max_depth_(max_depth),
      ...
      n_nodes_activated_(1),
      max_elements_per_leaf_(max_elem_per_leaf) {
  std::cout << "sizeof(Node): " << sizeof(Quadtree::Node) << "Bytes"
            << std::endl;

  // Make nodes
  n_nodes_total_ = (std::pow(4, max_depth_ + 1) - 1) / (4 - 1);
  nodes_.resize(n_nodes_total_ + 1);
  std::cout << "Max depth: " << max_depth_ << ", total bytes : "
            << (sizeof(Quadtree::Node) * n_nodes_total_) / 1024.0 << std::endl;
  ...
  float x_normalizer_ =
      static_cast<float>(nodes_[1].rect.br.x) / (x_range_[1] - x_range_[0]);
  float y_normalizer_ =
      static_cast<float>(nodes_[1].rect.br.y) / (y_range_[1] - y_range_[0]);
...
void Quadtree::insert(float x, float y, int id_point) {
  if (x > x_range_[0] && x < x_range_[1] && y > y_range_[0] &&
    ...
    this->insertPrivateStack(
        x_nom, y_nom,
        input_points_.size() - 1);  // Stack-based approach (DFS), faster than
                                    // Recursion-based one 1.2 times.
  } else
    throw std::runtime_error("Input x,y is out of the quadtree range.");
}

void Quadtree::insertPrivateStack(const float x_nom, const float y_nom,
                                  const ID id_elem) {
  ID index_node = 1;
  nodes_[1].depth = 0;
...
inline void Quadtree::makeChildrenLeaves(ID id_child,
                                         const Rectangle<_numeric_uint>& rect) {
  _numeric_uint cent_x = (rect.tl.x + rect.br.x) >> 1;
  _numeric_uint cent_y = (rect.tl.y + rect.br.y) >> 1;

  Node& nd0 = nodes_[id_child];
  nd0.makeLeaf();  // (0,0) (top left)
  nd0.rect.tl.x = rect.tl.x;
  nd0.rect.tl.y = rect.tl.y;
  nd0.rect.br.x = cent_x;
  nd0.rect.br.y = cent_y;

  Node& nd1 = nodes_[++id_child];
  nd1.makeLeaf();  // (0,1) (top right)
  nd1.rect.tl.x = cent_x;
  nd1.rect.tl.y = rect.tl.y;
  nd1.rect.br.x = rect.br.x;
  nd1.rect.br.y = cent_y;

  Node& nd2 = nodes_[++id_child];
  nd2.makeLeaf();  // (1,0) (bot left)
  nd2.rect.tl.x = rect.tl.x;
  nd2.rect.tl.y = cent_y;
  nd2.rect.br.x = cent_x;
  nd2.rect.br.y = rect.br.y;

  Node& nd3 = nodes_[++id_child];
  nd3.makeLeaf();  // (1,1) (bot right)
  nd3.rect.tl.x = cent_x;
  nd3.rect.tl.y = cent_y;
  nd3.rect.br.x = rect.br.x;
  nd3.rect.br.y = rect.br.y;
}

inline void Quadtree::makeBranch(ID id_node) {
  nodes_[id_node].makeBranch();
  nodes_bin_[id_node].reset();
}

inline bool Quadtree::BWBTest(float x, float y,
                              const Rectangle<_numeric_uint>& rect,
                              float radius) {
  // Ball Within Bound (Circle bounded by rect check)
  float rect_size = (float)(rect.br.x - rect.tl.x);
  return (rect_size >= 2.0f * radius) &&
         INBOUND_RECT_PTS(x, y, rect.tl.x + radius, rect.tl.y + radius,
                          rect.br.x - radius, rect.br.y - radius);
}

inline bool Quadtree::BOBTest(float x, float y,
                              const Rectangle<_numeric_uint>& rect, float R) {
  ...
  if (dx > (rect_halfsize_x + R) || dy > (rect_halfsize_y + R)) return false;

  if (dx <= rect_halfsize_x || dy <= rect_halfsize_y) return true;
...
inline void Quadtree::resetNNParameters() {
  // Initialize values
  simple_stack_.clear();
...
void Quadtree::nearestNeighborSearchPrivate() {
  ...
  while (!simple_stack_.empty()) {
    ...
    if (nd.isLeaf()) {
      ...
      if (nodes_[id_child].isActivated() &&
          BOBTest(x_nom, y_nom, nodes_[id_child].rect, query_data_.min_dist_)) {
...
bool Quadtree::findNearestElem(float x, float y,
                               const NodeBin& elems_thisnode) {
...
void Quadtree::cachedNearestNeighborSearchPrivate() {
  ...
  } else  // cached node is neither a root node nor a invalid node.
  {
    ...
    if (BWBTest(x_nom, y_nom, nodes_[query_data_.id_node_matched].rect,
    ...
    while (!simple_stack_.empty()) {
      ...
      if (nd.isLeaf()) {
        ...
        ID id_child = GET_FIRST_CHILD_ID(id_node);
        if (nodes_[id_child].isActivated() &&
            BOBTest(x_nom, y_nom, nodes_[id_child].rect,
                    query_data_.min_dist_)) {
          simple_stack_.push(id_child);
        }
        ++id_child;
...
void Quadtree::NNSearch(float x, float y, ID& id_point_matched,
                        ID& id_node_matched) {
  query_data_.x = x;
  query_data_.y = y;
...
void Quadtree::NNSearchDebug(float x, float y, ID& id_point_matched,
...
const size_t Quadtree::getNumNodesTotal() const { return n_nodes_total_; }
