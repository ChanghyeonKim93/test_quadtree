...
#define DIST_EUCLIDEAN(x, y, x_, y_) \
  (((x) - (x_)) * ((x) - (x_)) + ((y) - (y_)) * ((y) - (y_)))
#define DIST_MANHATTAN(x, y, x_, y_) (abs((x) - (x_)) + abs((y) - (y_)))

// Bound check functions
#define INBOUND_RECT(x, y, rect)                                   \
  (((x) > (rect).tl.x && (x) < (rect).br.x && (y) > (rect).tl.y && \
    (y) < (rect).br.y))
#define INBOUND_RECT_PTS(x, y, x_tl, y_tl, x_br, y_br) \
  (((x) > (x_tl) && (x) < (x_br) && (y) > (y_tl) && (y) < (y_br)))
...
#define GET_CHILD_ID_FLAGS(id_node, sn, ew) \
  (((id_node) << 2) - 2 + ((sn) << 1) + (ew))
#define GET_CHILD_ID_INDEX(id_node, idx_child) \
  (((id_node) << 2) - 2 + (idx_child))
#define GET_MY_QUADRANT(id_node) (((id_node) + 2) & 0b11)

#define FIND_QUADRANT(x_, y_, qrect, sn, ew)            \
  {                                                     \
    (ew) = (x_) > (((qrect).tl.x + (qrect).br.x) >> 1); \
    (sn) = (y_) > (((qrect).tl.y + (qrect).br.y) >> 1); \
  }
...
/*
   ...
   ...01010101010101010101010101010101 (32 bits) i = (i & 0x33333333) + ((i >>
   2) & 0x33333333);  // ...00110011001100110011001100110011 (32 bits) i = (i &
   0x0F0F0F0F) + ((i >> 4) & 0x0F0F0F0F);  //
   ...00001111000011110000111100001111 (32 bits) i = (i & 0x00FF00FF) + ((i >>
   8) & 0x00FF00FF);  // ...00000000111111110000000011111111 (32 bits) i = (i &
   0x0000FFFF) + ((i >> 16) & 0x0000FFFF); //
   ...00000000000000001111111111111111 (32 bits) return i & 0x3F;
    };
...
would append another element, 0xFFFFFFFF00000000, to b, append 32 to S, and loop
from 5 to 0. This method is much slower than the earlier table-lookup version,
but if you don't want big table or your architecture is slow to access memory,
it's a good choice. The second variation involves slightly more operations, but
it may be faster on machines with high branch costs (e.g. PowerPC).
...
0x00FF00FF}; static const unsigned int S[] = {1, 2, 4, 8};

    unsigned int x = 0b1011; // Interleave lower 16 bits of x and y, so the bits
of x unsigned int y = 0b0011; // are in the even positions and bits from y in
the odd; unsigned int z; // z gets the resulting 32-bit Morton Number.
...
// top    = ((( (z) & 0b10101010) − 1) & 0b10101010) | ( (z) & 0b01010101) // y
에서 1을 빼고, 다시 합쳐줌. y 추출, -1 후 다시 y로 만들어줌.

// bottom = ((( (z) | 0b01010101) + 1) & 0b10101010) | ( (z) & 0b01010101) // y
에서 1을 더하고, 다시 합쳐줌.
