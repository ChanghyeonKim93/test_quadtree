...
namespace quadtree {
std::unique_ptr<ObjectPool<Node>> Quadtree::objpool_node_ =
    std::make_unique<ObjectPool<Node>>(500000);
std::unique_ptr<ObjectPool<LinkedElement>> Quadtree::objpool_elem_ =
    std::make_unique<ObjectPool<LinkedElement>>(3000000);

Quadtree::Quadtree(const float x_min, const float x_max, const float y_min,
                   const float y_max, uint32_t max_depth,
                   uint32_t max_elem_per_leaf,
  ...
  x_normalizer_ =
      static_cast<float>(root_node_->rect.br.x) / (x_range_[1] - x_range_[0]);
  y_normalizer_ =
      static_cast<float>(root_node_->rect.br.y) / (y_range_[1] - y_range_[0]);
  ...
  parameters_.distance_approximate_rate =
      parameters_.distance_approximate_rate *
      parameters_.distance_approximate_rate;
...
void Quadtree::InsertReferenceData(const float x, const float y,
                                   const int data_id) {
  if (x < x_range_[0] || x > x_range_[1]) return;
  if (y < y_range_[0] || y > y_range_[1]) return;
      ...
      if (depth < max_depth_) {
        ...
        if (n_elem > max_elem_per_leaf_) {
          ...
          while (element != nullptr) {
            ...
            Flag flag_sn, flag_ew;
            FIND_QUADRANT(element->normalized_x, element->normalized_y,
                          node->rect, flag_sn, flag_ew);
            uint8_t child_index = GET_CHILD_INDEX(flag_sn, flag_ew);
...
void Quadtree::SearchNearestNeighbor(const float x, const float y,
                                     ID& matched_data_id,
...
void Quadtree::SearchNearestNeighbor_debug(const float x, const float y,
                                           ID& matched_data_id,
                                           NodePtr& matched_node_ptr,
                                           uint32_t& num_node_access) {
...
void Quadtree::SearchNearestNeighborWithNodeCache_debug(
  ...
  SearchNearestNeighborWithNodeCache(x, y, cached_node_ptr, matched_data_id,
                                     matched_node_ptr);
  num_node_access = simple_stack_.getTotalAccess();
}

inline void Quadtree::AddDataToNode(NodePtr node_ptr,
                                    LinkedElementPtr element_ptr) {
  element_ptr->next = nullptr;
  if (node_ptr->element_head != nullptr) {  // not empty
...
bool Quadtree::FindNearestElement(const float x, const float y,
                                  NodePtr node_ptr) {
  bool is_new_nearest_found = false;
  LinkedElementPtr element_ptr = node_ptr->element_head;
  while (element_ptr != nullptr) {
    float current_distance_squared = DIST_EUCLIDEAN(
        x, y, element_ptr->normalized_x, element_ptr->normalized_y);
    if (current_distance_squared < query_data_.min_distance_sqaured_) {
      // std::cout << "min dist chaged: " << query_data_.min_dist2_ <<","
      // <<dist_temp<<std::endl;
      query_data_.matched_data_id = element_ptr->data_id;
      query_data_.min_distance_sqaured_ =
          current_distance_squared * parameters_.distance_approximate_rate;
...
void Quadtree::SearchNearestNeighborWithNodeCache(const float x, const float y,
...
void Quadtree::SearchNearestNeighborPrivate() {
  ...
  while (!simple_stack_.empty()) {
    ...
    if (node_ptr->IsLeaf()) {
      ...
      if (CheckBallWithinBound(normalized_x, normalized_y, node_ptr->rect,
                               query_data_.min_distance_))
      ...
      if (ptr_child->IsActivated() &&
          CheckBallOverlapBound(normalized_x, normalized_y, ptr_child->rect,
                                query_data_.min_distance_)) {
...
void Quadtree::SearchNearestNeighborWithNodeCachePrivate() {
  ...
  } else  // cached node is neither a root node nor a invalid node.
  {
    ...
    FindNearestElement(normalized_x, normalized_y,
                       query_data_.matched_node_ptr);
    if (CheckBallWithinBound(normalized_x, normalized_y,
                             query_data_.matched_node_ptr->rect,
                             query_data_.min_distance_))
...
inline bool Quadtree::CheckBallWithinBound(const float x, const float y,
                                           const Rect_u& rect,
                                           const float radius) {
...
inline bool Quadtree::CheckBallOverlapBound(const float x, const float y,
                                            const Rect_u& rect,
                                            const float radius) {
  ...
  if (dx > (rect_halfsize_x + radius)) return false;
  if (dy > (rect_halfsize_y + radius)) return false;

  if (dx <= rect_halfsize_x) return true;
  if (dy <= rect_halfsize_y) return true;

  const auto corner_dist_sq = (dx - rect_halfsize_x) * (dx - rect_halfsize_x) +
...
uint32_t Quadtree::GetNumActivatedNodes() { return n_node_activated_; }

void Quadtree::GetAllElementInRoot() {
  LinkedElementPtr element_ptr = root_node_->element_head;
  while (element_ptr != nullptr) element_ptr = element_ptr->next;
}
};  // namespace quadtree