...
#define SET_DEPTH(state,d)   {(state) &= ~0b00001111; (state) |= static_cast<uint8_t>((d));}
#define GET_DEPTH(state)   (static_cast<uint16_t>((state) & 0b00001111)) // lower four bytes
#define PLUS_DEPTH(state)   ((state) += 1)
#define MINUS_DEPTH(state)  ((state) -= 1)

#define MAKE_UNACTIVATED(state)   {(state) &= ~BIT_BRANCH; (state) &= ~BIT_LEAF;}
#define MAKE_LEAF(state)   {(state) &= ~BIT_BRANCH; (state) |= BIT_LEAF;}
#define MAKE_BRANCH(state) {(state) &= ~BIT_LEAF; (state) |= BIT_BRANCH;}
...
namespace HashBased2
{
  ...
  private: // Related to generate tree.
    ...
    void addNewNode(const uint32_t node_id, const uint8_t depth, const Rectangle<uint16_t>& rect);
    Rectangle<uint16_t> getSubRectangle(const Rectangle<uint16_t>& rect, const bool flag_sn, const bool flag_ew);
