...
Quadtree::Quadtree(float x_min, float x_max, float y_min, float y_max,
                   std::shared_ptr<ObjectPool<QuadNode>> objpool_node,
                   std::shared_ptr<ObjectPool<Elem>> objpool_elem,
    ...
    if (IS_BRANCH_P(ptr_node)) {
      ...
      if (depth < max_depth_) {
        ...
        if (n_elem > max_elem_per_leaf_) {
          ...
          // Do divide
          for (ElemPtr const& elem_tmp :
               vector_elemptrs_[ptr_node->id_elemptrs].elems) {
            Flag flag_sn, flag_ew;
            FIND_QUADRANT(elem_tmp->x_nom, elem_tmp->y_nom, ptr_node->rect,
                          flag_sn, flag_ew);
            uint8_t idx_child = GET_CHILD_INDEX(flag_sn, flag_ew);
...
void Quadtree::cachedNNSearchDebug(float x, float y,
                                   QuadNodePtr ptr_node_cached,
                                   ID& id_data_matched,
                                   QuadNodePtr& ptr_node_matched,
                                   uint32_t& n_access) {
...
inline void Quadtree::makeChildrenLeaves(QuadNodePtr ptr_parent) {
  ...
  ptr_child->rect.br.x = cent_x;
  ptr_child->rect.br.y = cent_y;
...
void Quadtree::nearestNeighborSearchPrivate() {
  ...
  while (!simple_stack_.empty()) {
    ...
    if (IS_LEAF_P(ptr_node)) {
      ...
      if (BWBTest(x_nom, y_nom, ptr_node->rect, query_data_.min_dist_))
      ...
      if (IS_ACTIVATED_P(ptr_child) &&
          BOBTest(x_nom, y_nom, ptr_child->rect, query_data_.min_dist_)) {
...
void Quadtree::cachedNearestNeighborSearchPrivate() {
  ...
  } else  // cached node is neither a root node nor a invalid node.
  {
    ...
    if (BWBTest(x_nom, y_nom, query_data_.ptr_node_matched->rect,
