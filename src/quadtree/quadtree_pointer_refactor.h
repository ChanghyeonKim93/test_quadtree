...
namespace quadtree
{
  ...
  { // 20 bytes (actually 16 bytes)
    ...
    LinkedElement(
        const float input_normalized_x, const float input_normalized_y, const int input_data_id)
        : next(nullptr), data_id(input_data_id), normalized_x(input_normalized_x), normalized_y(input_normalized_y) {}
    inline void ResetAndSetData(const float input_normalized_x, const float input_normalized_y, const int input_data_id)
    {
      next = nullptr;
...
#define MAKE_LEAF_P(nd) ((nd)->state = STATE_LEAF)
  ...
  private:
    struct InsertData
    {                               // 16 bytes (actually 16 bytes)
      float normalized_x;           // 4 bytes
      float normalized_y;           // 4 bytes
      LinkedElementPtr element_ptr; // 8 bytes

      InsertData() : normalized_x(-1.0f), normalized_y(-1.0f), element_ptr(nullptr){};
      void ResetAndSetData(const float x_nom_in, const float y_nom_in, LinkedElementPtr elem_)
      {
