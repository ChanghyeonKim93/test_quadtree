...
class Quadtree {
 ...
 public:
  Quadtree(float x_min, float x_max, float y_min, float y_max,
           ...
           std::shared_ptr<ObjectPool<Elem>> objpool_elem, uint32_t max_depth,
           uint32_t max_elem_per_leaf, float approx_rate = 1.0,
           uint8_t flag_adj = false);
  ~Quadtree();

  void insert(float x, float y, int id_data);
  void NNSearch(float x, float y, ID& id_data_matched,
  ...
  void NNSearchDebug(float x, float y, ID& id_data_matched,
                     QuadNodePtr& ptr_node_matched, uint32_t& n_access);
