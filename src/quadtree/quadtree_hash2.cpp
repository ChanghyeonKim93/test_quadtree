...
Quadtree::Quadtree(float x_min, float x_max, float y_min, float y_max,
  ...
  Rectangle<_numeric_uint> rect_root = Rectangle<_numeric_uint>(
      Corner<_numeric_uint>(0, 0), Corner<_numeric_uint>(br_x, br_y));

  this->addNewNode(node_id_root, depth_root, rect_root);  // root node

  // Calculate normalizers
  float x_normalizer_ =
      static_cast<float>(rect_root.br.x) / (x_range_[1] - x_range_[0]);
  float y_normalizer_ =
      static_cast<float>(rect_root.br.y) / (y_range_[1] - y_range_[0]);
...
void Quadtree::insertPrivateStack(const float x_nom, const float y_nom,
  ...
  while (true) {
    ...
    const Rectangle<uint16_t>& rect_to_visit =
        node_list_[node_id_to_visit].getRect();
    ...
    if (node.isBranch()) {
      ...
      if (node_list_.count(child_id) == 0) {
        this->addNewNode(
            child_id, depth + 1,
            this->getSubRectangle(rect_to_visit, flag_sn, flag_ew));
      }
      ...
      if (depth < this->max_depth_) {
        ...
        if (n_elem > this->max_elements_per_leaf_) {
          ...
          for (const ID& elem_id : node_elem_id_list_[node_id_to_visit]) {
            ...
            FIND_QUADRANT(elem.x_nom, elem.y_nom, rect_to_visit, flag_sn,
                          flag_ew);
            ID child_id =
                GET_CHILD_ID_FLAGS(node_id_to_visit, flag_sn, flag_ew);
...
Quadtree::Rectangle<uint16_t> Quadtree::getSubRectangle(
    const Quadtree::Rectangle<uint16_t>& rect, const bool flag_sn,
    const bool flag_ew) {
