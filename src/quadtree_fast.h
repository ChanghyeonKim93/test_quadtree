...
#define MAKE_LEAF(nd) ((nd).state = STATE_LEAF)
 ...
 public:
  ...
  ID cachedNNSearchDebug(float x, float y, int id_node_cached,
                         uint32_t& n_access);

  // Related to generate tree.
 private:
  void insertPrivate(ID id_node, uint8_t depth);
  inline void getQuadrant(float x, float y, const QuadRect_u16& qrect,
                          Flag& flag_we, Flag& flag_ns);
  inline void getQuadrantRect(Flag flag_we, Flag flag_ns,
                              const QuadRect_u16& qrect,
 ...
 private:
  ...
  inline bool BWBTest(float x, float y, const QuadRect_u16& rect,
                      float radius);  // Ball Within Bound
  inline bool BOBTest(float x, float y, const QuadRect_u16& rect,
