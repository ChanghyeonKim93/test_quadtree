...
int main() {
  ...
  try {
    ...
    objpool_node = std::make_shared<ObjectPool<PointerBased2::QuadNode>>(
        16 * 100000);  // 이만큼이 될 리가 ...?
    objpool_elem =
        std::make_shared<ObjectPool<PointerBased2::Elem>>(16 * 100000);  //
    ...
    for (int d = 0; d <= max_depth; ++d) {
      ...
      qt = std::make_shared<Quadtree>(x_range[0], x_range[1], y_range[0],
                                      y_range[1], objpool_node, objpool_elem, d,
                                      max_elem_per_leaf, approx_rate);
      ...
      std::cout << "# nodes - activated: " << qt->getNumNodesActivated()
                << std::endl;
      ...
      for (uint32_t i = 0; i < n_pts_q; ++i) {
        ...
        qt->NNSearchDebug(pts_q[i].first, pts_q[i].second, id_data_matched_q[i],
                          ptr_node_matched_q[i], access_temp);
      ...
      for (int i = 0; i < n_pts_q; ++i) {
        ...
        qt->cachedNNSearchDebug(pts_q[i].first, pts_q[i].second,
                                ptr_node_matched_q[i], id_data_matched_q[i],
                                ptr_node_matched_q[i], access_temp);
