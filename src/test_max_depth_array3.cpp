...
int main() {
  ...
  for (size_t i = 0; i < n_pts_q; ++i) {
    pts_q.push_back(std::make_pair<float, float>(generator(), generator()));
  }

  std::vector<size_t> id_node_matched_q;
  std::vector<size_t> id_point_matched_q;
  id_node_matched_q.resize(n_pts_q);
  id_point_matched_q.resize(n_pts_q);

  std::vector<double> min_dist2_true;
  std::vector<size_t> id_point_matched_q_true;
  min_dist2_true.resize(n_pts_q);
  id_point_matched_q_true.resize(n_pts_q);

  for (size_t i = 0; i < n_pts_q; ++i) {
    double min_dist2 = std::numeric_limits<double>::max();
    size_t id_point_tmp = 0;
    ...
    id_point_matched_q_true[i] = id_point_tmp;
  }
  ...
  try {
    ...
    std::vector<size_t> access_normal(max_depth + 1);
    ...
    std::vector<size_t> access_cached(max_depth + 1);
    ...
    std::vector<size_t> diff_normal(max_depth + 1);
    std::vector<size_t> diff_cached(max_depth + 1);

    for (int d = 0; d <= max_depth; ++d) {
      timer::tic();
      std::shared_ptr<Quadtree> qt = nullptr;
      qt = std::make_shared<Quadtree>(x_range[0], x_range[1], y_range[0],
                                      y_range[1], d, max_elem_per_leaf,
                                      approx_rate);
      ...
      timer::tic();
      for (size_t i = 0; i < n_pts; ++i) {
        qt->insert(pts_data[i].first, pts_data[i].second, i);
      }

      std::cout << "# nodes - total / activated / ratio: "
                << qt->getNumNodesTotal() << ", " << qt->getNumNodesActivated()
                << ", "
                << (double)qt->getNumNodesActivated() /
                       (double)qt->getNumNodesTotal() * 100.0
                << " % " << std::endl;
      ...
      timer::tic();
      for (size_t i = 0; i < n_pts_q; ++i) {
        size_t access_temp = 0;
        qt->NNSearchDebug(pts_q[i].first, pts_q[i].second,
                          id_point_matched_q[i], id_node_matched_q[i],
                          access_temp);
      ...
      double sq_sum = std::inner_product(accesses.begin(), accesses.end(),
                                         accesses.begin(), 0.0);
      ...
      for (int i = 0; i < n_pts_q; ++i) {
        const size_t& id_mat = id_point_matched_q[i];
        const size_t& id_mat_true = id_point_matched_q_true[i];

        if (id_mat != id_mat_true) {
          diff_normal[d]++;
          std::cout << "mismatched: " << id_mat << "," << id_mat_true << " / "
                    << pts_data[id_mat].first << "," << pts_data[id_mat].second
                    << " / " << pts_data[id_mat_true].first << ","
                    << pts_data[id_mat_true].second
                    << " / mindist2 (true,est):" << min_dist2_true[i] << ","
                    << DIST_EUCLIDEAN(pts_data[id_mat].first,
                                      pts_data[id_mat].second, pts_q[i].first,
                                      pts_q[i].second)
                    << std::endl;
        }
      }
      ...
      for (int i = 0; i < n_pts_q; ++i) {
        size_t access_temp = 0;
        qt->cachedNNSearchDebug(pts_q[i].first, pts_q[i].second,
                                id_node_matched_q[i], id_point_matched_q[i],
                                id_node_matched_q[i], access_temp);
      ...
      for (int i = 0; i < n_pts_q; ++i) {
        size_t id_mat = id_point_matched_q[i];
        size_t id_mat_true = id_point_matched_q_true[i];
      ...
      sq_sum = std::inner_product(accesses.begin(), accesses.end(),
                                  accesses.begin(), 0.0);
    ...
    for (int d = 0; d <= max_depth; ++d) {
      std::cout << " d[" << d << "] - " << "build: " << time_construct[d]
                << ", " << "insert: " << time_insert[d] << ", "
                << "NN search (normal/cached): " << time_normal[d] << ", "
                << time_cached[d] << "\n";
    }
    ...
    for (int d = 0; d <= max_depth; ++d) {
      std::cout << " d[" << d << "] - (normal/cached)"
                ...
                << " / " << "min: " << min_access_normal[d] << ","
                << min_access_cached[d] << " / "
                << "max: " << max_access_normal[d] << ","
                << max_access_cached[d] << " / "
                << "avg: " << avg_access_normal[d] << ","
                << avg_access_cached[d] << " / "
                << "std: " << std_access_normal[d] << ","
                << std_access_cached[d] << std::endl;
    }
