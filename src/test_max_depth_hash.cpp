...
int main() {
  ...
  try {
    ...
    for (int d = 0; d <= max_depth; ++d) {
      ...
      std::cout << "depth: " << d
                << ", # nodes - activated: " << qt->getNumNodesActivated()
                << "\n";

      std::cout << "time : " << time_insert[d] << " [ms]" << std::endl;
      ...
      //         id_point_matched_q[i], id_node_matched_q[i], access_temp);
      ...
      // accesses.begin(), 0.0); double stdev = std::sqrt(sq_sum /
      // accesses.size() - mean * mean); avg_access_normal[d] = mean;
      ...
      //         DIST_EUCLIDEAN(pts_data[id_mat].first, pts_data[id_mat].second,
      //         pts_q[i].first, pts_q[i].second) << std::endl;
      //     }
      // }
      ...
      // accesses.begin(), 0.0); stdev = std::sqrt(sq_sum / accesses.size() -
      // mean * mean); avg_access_cached[d] = mean; std_access_cached[d] =
      // stdev;
    }
    ...
    //     / "
    //     << "std: " << std_access_normal[d] <<"," << std_access_cached[d] <<
    //     std::endl;
    // }
